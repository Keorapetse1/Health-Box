...
// LCD Setup - I2C address may vary (common addresses: 0x27 or 0x3F)
...
void setup() {
  ...
  // Initialize I2C communication for temperature sensor and LCD
  Wire.begin();
...
void updateLCD() {
  ...
  if (millis() - lastLCDUpdate >= lcdUpdateInterval) {
    ...
    if (beatDetected && (millis() - lastBeatTime < 5000)) {
      ...
      lcd.print("--  "); // Show -- when no recent heartbeat
      beatDetected = false;
    }
    
    // Update temperature on second line
    lcd.setCursor(0, 1);
    lcd.print("Temp: ");
    lcd.print(objectTemp, 1); // Show temperature with 1 decimal place
    lcd.print("C");
    lcd.print("   "); // Clear any leftover characters
...
void connectToWiFi() {
  ...
  // Update LCD to show connection status
  lcd.clear();
  lcd.setCursor(0, 0);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Update LCD to show connection successful
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("WiFi Connected");
    lcd.setCursor(0, 1);
    lcd.print("IP: ");
    lcd.print(WiFi.localIP().toString().substring(0, 8)); // Show first part of IP
    delay(2000); // Show for 2 seconds
  } else {
    Serial.println("");
    Serial.println("Failed to connect to WiFi. Please check credentials.");
    
    // Update LCD to show connection failed
    lcd.clear();
    lcd.setCursor(0, 0);
